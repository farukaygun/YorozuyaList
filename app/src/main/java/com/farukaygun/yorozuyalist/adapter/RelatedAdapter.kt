...
class RelatedAdapter(val type: Int, private val relatedList: List<Related>) :
	...
	override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
		val binding = DataBindingUtil.inflate<ItemHomeAnimeRecyclerBinding>(
	...
	override fun onItemClicked(view: View, data: Node) {
		...
		when (type) {
			0 -> Navigation.findNavController(view).navigate(
				AnimeDetailsFragmentDirections.actionAnimeDetailsFragmentSelf().actionId,
				bundle
			)

			1 -> Navigation.findNavController(view).navigate(
				MangaDetailsFragmentDirections.actionMangaDetailsFragmentSelf().actionId,
				bundle
			)
		}
	}
